...
This study is a Problem Set of the "Data Analysis with R" course, part of Udacity's Data Analyst Nanodegree curriculum. In this study, we select two datasets from [Gapminder](www.gapminder.org/data) and perform exploratory analyses using different types of visualisations.  
...
To get a first idea of the data, we compute some summary statistics and plot the data distribution for 1950 and 2016 (first and latest year in the sample):
```{r}
...
Moreover, every single country has improvemed their LEB:
...
We then build the same frequency polygon plot as with the previous dataset, selecting a few years:
...
As with Life Expectancy at Birth, the biggest gains happened in the poorer countries. The lowest group however is still far from the levels reached by the other countries and still has more variance.
...
Here, each observation or dot on the plot is a combination of a country and a year.
...
From all the observations above, we can conclude that a high immunization ratio is associated with:
 ...
 - A slightly negative skewness that means that a minority of countries/year pairs are lowering the mean by significantly lower LEB values, but the majority is actually above the mean. We can clearly see a number of outliers in the lower-right part of the plot (between 80% and 100% immunization and 40 to 55 years old, approximately).

## Recap: Observations on Immunization vs. Life Expectancy

In this part of the analysis, we made the following observations:

 - The DTP3 Immunization percentage among 1-year-old followed a similar pattern to Life Expectancy at Birth: A rapid progress since 1980, a reduction in the variance accross countries and poorer countries catching up with rich ones,
 - There is a robust positive correlation (over .6) between Immunization and Life Expectancy,
 - Where and when immunization is high, we tend to have the majority of observations above the LEB average. Although a few outliers tend to pull the mean down, this is a better situation than the opposite. These outliers could be due to one-off events (such a natural disasters, wars, epidemics) which, although tragic, are unlikely to occur on a regular basis.

# Conclusion

Overall this analysis shows just how much public health has improved since the middle of the 1950, especially in poorer countries. Life Expectancy at Birth and DTP3 Immunization Ratio among 1-year-olds are just two metrics. The correlation between them is clear and could probably be found also with other social metrics such as litteracy. Together, they paint a rather promising picture for the world's future.